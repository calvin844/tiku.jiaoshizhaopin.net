    ...
    <div class="main_box exercise">
        <div class="list_box favorite">
            ...
            <div class="content">
                        ...
                        <?php foreach ($user_collect as $u): ?>
                            <?php if ($u['info']['parent_id'] == 0): ?>
                                <li class="level1" data-id="<?= $u['info']['id'] ?>" parent-id="0">
                                    ...
                                    <div class="right">
                                        <a href="/tiku/show_collection?id=<?= $u['info']['id'] ?>-<?= trim($u['question_id'], ",") ?>">查看题目</a>
                                    </div>
                                </li>
                                <?php foreach ($user_collect as $u2): ?>
                                    <?php if ($u2['info']['parent_id'] == $u['info']['id']): ?>
                                        <li class="level2" data-id="<?= $u2['info']['id'] ?>" parent-id="<?= $u2['info']['parent_id'] ?>">
                                            ...
                                            <div class="right">
                                                <a href="/tiku/show_collection?id=<?= $u2['info']['id'] ?>-<?= trim($u2['question_id'], ",") ?>">查看题目</a>
                                            </div>
                                        </li>
                                        <?php foreach ($user_collect as $u3): ?>
                                            <?php if ($u3['info']['parent_id'] == $u2['info']['id']): ?>
                                                <li class="level3" data-id="<?= $u3['info']['id'] ?>" parent-id="<?= $u3['info']['parent_id'] ?>">
                                                    ...
                                                    <div class="right">
                                                        <a href="/tiku/show_collection?id=<?= $u3['info']['id'] ?>-<?= trim($u3['question_id'], ",") ?>">查看题目</a>
                                                    </div>
                                                </li>
                                            <?php endif; ?>
                                        <?php endforeach; ?>
                ...
                <?php else: ?>
                    <p class="tips">还没有自己的收藏本？&nbsp;<a target="_self" href="/tiku_<?= $_SESSION['house_name'] ?>">点击这里</a>&nbsp;，马上打造您的专属练习本！</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
