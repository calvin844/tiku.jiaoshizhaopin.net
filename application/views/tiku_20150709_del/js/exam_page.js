...
$('div.list_box ul.left_box li a.next_time').click(function() {
    var a = 0;
    var answer_str = "";
    $('div.answers li').each(function() {
        if ($(this).find('span.active').is(":visible")) {
            a++;
            var question_id = $(this).parents('div.answers').find('a').attr('favor-id');
            var question_score = $(this).parents('.item_box').attr('score');
            answer_str += question_id + "-" + $(this).attr('answer-id') + "-" + question_score + "##";
        }
    })
...
$('div.right_box div.question_box div.answers ul li').live('click', function() {
    ...
    $(this).parents('div.item_box').attr('user-answer', $(this).attr('answer-id'));
