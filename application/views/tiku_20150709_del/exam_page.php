    ...
    <div class="main_box">
        <div class="list_box">
            <ul class="left_box">
                ...
                <li>
                    <a class="time_pause" href="javascript:void(0);" title="暂停">暂停</a>
                </li>
                <li>
                    <a class="next_time" href="javascript:void(0);" title="下次再做">下次再做</a>
                </li>
                <li>
                    <a class="submit" href="javascript:void(0);" title="交卷">交卷</a>
                </li>
                <li class="code">
                    <p>轻松考教师就扫我！</p>
                    ...
                    <ul>
                        ...
                        <?php foreach ($exam_paper as $k => $v): ?>
                            <li <?php if ($k == 'part_1'): ?>class="cur"<?php endif; ?>>
                                <a class="chapter-switch" href="javascript:void(0);" id="<?= $k ?>"><?= $v['name']; ?> [<label class="answer-count">0</label>/<?= $v['question_num']; ?>]</a>
                                <span></span>
                            </li>
                        <?php endforeach; ?>
                        <!--循环试题类型选项卡 end-->
                                ...
                                <?php foreach ($exam_paper as $k => $v): ?>
                                    <?php if (strstr($k, "part_")): ?>
                                        <?php foreach ($v['question_result'] as $v): ?>
                                            <a part-data="<?= $k ?>" question-data="question_<?= $v['id'] ?>" class="<?php if ($v['question_type_id'] != "3" && $v['question_type_id'] != "4" && $v['question_type_id'] != "6" && $v['question_type_id'] != "7" && $v['question_type_id'] != "8" && $v['question_type_id'] != "9"): ?>answers_a<?php endif; ?>" href="javascript:void(0);"><?= $l; ?></a>
                                            <?php if ($l % 5 == 0): ?>
                                            </li>
                                            <li>
                                            <?php endif; ?>
...
<div class="time_pause_box">
    <div class="bg">
        <a href="javascript:void(0);">继续做题</a>
    </div>
</div>
<!-- 暂停倒时计窗口 end -->
...
<div class="submit_box">
    <div class="bg">
        <p>确定要交卷吗？</p>
        <a class="sure" href="javascript:void(0)">确定交卷</a>
        <a class="cancel" href="javascript:void(0)">谢谢提醒，继续做</a>
    </div>
</div>
<!-- 交卷提示窗口 end -->
...
<div class="big_material_box">
    <div class="big_material">
        <a class="m_close" href="javascript:void(0);"></a>
...
<form id="submit_form" method="post" action="/tiku/submit_exam">
    <input type="hidden" name="qs_user_id" value="<?= $_SESSION['qs_user_id']; ?>" />
