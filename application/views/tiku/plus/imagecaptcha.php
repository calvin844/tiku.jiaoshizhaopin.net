...
$path = explode("/", $path);
session_save_path('/data/www/'.$path[3].'/system/public/sess_save_path');
session_start();
...
class imageCaptcha {
    ...
    public function imageCaptcha() {
        ...
        $this->rootpath = pathinfo(__FILE__, PATHINFO_DIRNAME) . "/";
    }

    public function set_show_mode() {
        require_once($this->rootpath . "cache_captcha.php");
        $this->cfg = $data;
...
if ($act == "verify") {
    ...
    if (preg_match("/^[A-Za-z0-9]*$/", $c)) {
        ...
        $rootpath = VIEW_PATH . "tiku/plus/";
        require_once($rootpath . 'config.php');
        if (strcasecmp("utf8", "utf8") != 0) {
            $c = iconv("utf-8", "utf8", $c);
        }
    }
