...
if ($('div.exercise ul#navLink li.cur a').length > 0) {
    var n1 = $('div.exercise ul#navLink li.cur a').html();
    $('head title').html(n1 + "-" + f_year + "年教师资格证考试-教师招聘网");
} else if ($('div.vip form#sure_order').length > 0) {
    $('head title').html("充值中心-" + f_year + "年教师资格证考试-教师招聘网");
} else if ($('div.header div.bottom li.hover a').length > 0 && $('div.list_box div.right_box p.top_title').length > 0) {
    var n1 = $('div.header div.bottom li.hover a').html();
    var n2 = $('div.list_box div.right_box p.top_title').html();
    $('head title').html(n2 + "-" + f_year + "年" + n1 + "教师资格证考试-教师招聘网");
} else if ($('div.header div.bottom li.hover a').length > 0) {
    var n1 = $('div.header div.bottom li.hover a').html();
    $('head title').html(f_year + "年" + n1 + "教师资格证考试-教师招聘网");
}
设置页面标题 end*/

/*
$('a#js_check_login').live('click', function() {
    var u_id = $('div.header div.bottom div.right input#session_id').attr('value');
    var to_url = $(this).attr('u_data');
    if (u_id < 1) {
        $("div.phone_html").show();
        return false;
    } else {
